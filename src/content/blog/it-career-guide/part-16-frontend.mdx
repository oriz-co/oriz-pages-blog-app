...
```mermaid
graph TD
    subgraph Server Runtime (V8 Edge / Node)
        rsc["React Server Component<br>(Queries DB directly)"] -->|Generates JSON payload| stream["Stream to Client"]
    end
    subgraph Client Browser
        stream -->|Hydrate Interactive Nodes| clientComp["Client Component<br>(Interactive JS)"]
    end
```
