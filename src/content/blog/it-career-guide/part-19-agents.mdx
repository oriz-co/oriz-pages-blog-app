...
```mermaid
graph LR
    subgraph HostApp ["Host Application: Cursor or VS Code or Claude Desktop"]
        MCPClient["MCP Client"]
    end
    subgraph MCPServerProc ["MCP Server: Local Process or Subprocess"]
        MCPServer["MCP Server Interface"] --> Resources["Resources: Local Files or DB"]
        MCPServer --> Tools["Tools: File edit or Run tests"]
        MCPServer --> Prompts["Prompts: Templates"]
    end
    MCPClient <-->|JSON-RPC 2.0 over stdio or HTTP| MCPServer
```
